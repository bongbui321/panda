...
class TestFordSafetyBase(common.PandaCarSafetyTest):
  ...
  @pytest.mark.parametrize('speed', [
      pytest.param(CURVATURE_ERROR_MIN_SPEED - 1, id="below_min_speed"),
      pytest.param(CURVATURE_ERROR_MIN_SPEED + 1, id="above_min_speed")
  ])
  @pytest.mark.parametrize('controls_allowed', [True, False])
  @pytest.mark.parametrize('steer_control_enabled', [True, False])
  @pytest.mark.parametrize('path_offset', np.arange(-5.12, 5.11, 1).round())
  @pytest.mark.parametrize('path_angle', np.arange(-0.5, 0.5235, 0.1).round(1))
  @pytest.mark.parametrize('curvature_rate', np.arange(-0.001024, 0.00102375, 0.001).round(3))
  @pytest.mark.parametrize('curvature', np.arange(-0.02, 0.02094, 0.01).round(2))
  def test_steer_allowed(self, speed, controls_allowed, steer_control_enabled,
      ...
      assert should_tx == self._tx(self._lat_ctl_msg(steer_control_enabled, path_offset,
