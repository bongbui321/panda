...
class TestPandaComms:
  def test_tx_queues(self):
    ...
    for bus in range(len(TX_QUEUES)):
      ...
      can_pkt_tx = libpanda_py.make_CANPacket(message[0], message[2], message[1], lpp)
  ...
  def test_comms_reset_rx(self):
    ...
    for _ in range(100):
      can_pkt_tx = libpanda_py.make_CANPacket(test_msg[0], test_msg[2], test_msg[1], lpp)
  ...
  def test_can_receive_usb(self):
    ...
    packets = [libpanda_py.make_CANPacket(m[0], m[2], m[1], lpp) for m in msgs]
